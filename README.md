...
ClippyManager is **paid up front on the Mac App Store** — there's no trial, no
in-app purchase, and no licensing code in the app. Anyone who launches it has
already paid, so every feature is unlocked from first run. (The hard paywall is
the App Store product itself.)
